...
| The Chris Rule (no auditioning for a show you're on the team of) | **LOCKED** | Product invariant. Enforced server-side in `signUpForAudition` (both modes) AND in the public audition page UI (QA fix 2026-07-05): viewers who pass `getShowAccess` (team OR org owner/admin) see an inert "You're on this show's production team" card instead of the signup CTA. |
...
| Navigation system (owner-authorized redesign 2026-07-05) | **LOCKED** | Three pieces, strict separation: **actor `Nav`** ((actor)+(public) groups) ALWAYS renders actor links (Discover / My Shows / Profile + bell + avatar) — it never reads `activeRole`, never shows a show menu (the old stale-team-role `teamLinks` leak is deleted; do not reintroduce). **`ProductionTopNav`** always shows persistent "My Shows" (/shows) + "My Theatre" (/org) links on EVERY production page, desktop AND mobile; inside a show it adds the title-labeled show switcher (own row on mobile) and `ProductionSubNav`. Theatre switcher (2026-07-06): when the user belongs to MORE THAN ONE theatre, "My Theatre" becomes a caret dropdown (same pattern as ShowSwitcher — org avatar + name + role, check on the selected; pick → `selectOrg` + `/org`) and the avatar dropdown lists each theatre; with one membership it stays a plain link (mock mode = always one). **`PreviewBanner`** on public show/theatre pages tells team/org viewers "this is what actors see" with a back link — never shown to actors/anon, silent-fails. Visiting public pages never changes role/context. QA fix 2026-07-05: when the signed-in user has NO org (`useOrg` → null, not loading), the avatar dropdown + mobile menu show a subtle "Start a theatre" entry (→ `/onboarding?path=maker`) where members see their org name. |
| `useOrg()` org identity (never hardcode `org-1`) | **LOCKED** | Every production page derives org from this hook. Multi-theatre aware (bug fix 2026-07-06 — members of 2+ theatres could only ever see their first): fetches ALL active memberships (`getMyOrgMemberships`, ordered by joined_at) and exposes `{ org, role, memberships, selectOrg, isLoading }`; `org`/`role` = the SELECTED membership (per-user localStorage key `overture.selectedOrgId.<userId>`, falls back to the first and re-stores if the saved id is stale), so existing consumers work unchanged. `selectOrg` persists + broadcasts via a shared Zustand store + invalidates all queries. Maker onboarding selects the freshly created theatre (`selectOrgForUser`). |
...
| "My Theatre" entry in actor nav for org members | **LOCKED** | The actor↔theatre bridge. Multi-org (2026-07-06): the avatar dropdown + mobile menu list EACH theatre by name (click = select that theatre → `/org`) instead of just the first. |
...
| Callback scheduling (date/time/location/notes) on setup | **LOCKED** | Bug fix 2026-07-05 (QA walkthrough): times are now built as LOCAL instants (`new Date(...).toISOString()`) before saving — the old naked `YYYY-MM-DDTHH:MM:00` string was read as UTC by timestamptz and shifted 6:30 PM to 2:30 PM. The day-based audition generator was already local-correct. |
...
| Share affordances on the auditions management page (`/shows/[showId]/auditions` header: "View public page" + "Copy link", same pattern as setup; hidden while the show is a `setup` draft) | **OPEN** | Owner-ordered change (2026-07-05) to the LOCKED auditions page — pending user verify. |
| Status step-back on setup (`PREV_STATUS`: auditions_closed→auditions_open "Reopen Auditions", callbacks→auditions_closed, casting→callbacks, archived→cast "Restore from Archive"; each behind the same confirm-modal pattern with an honest warning line, reusing `statusMutation`. `cast` deliberately has NO entry — its step-back button points to the existing Unpublish on the cast-list page, never duplicated) | **OPEN** | Owner-ordered change (2026-07-05) to the LOCKED setup page — pending user verify. Only statuses with a safe backward target get a "Step back" ghost button. |
...
| Org member invitations + auto-accept at invitee's next sign-in (in-app) | **LOCKED** | Email delivery is OPEN (Resend). QA fix 2026-07-05: the invitee's typed name is stored (`org_invites.invited_name`, migration 014, in `PASTE_ME_NEXT.sql`) and shown on pending rows instead of the email prefix; degrades gracefully pre-paste. |
| Archive show | **LOCKED** | |
...
| Public audition slot visibility — EVERYONE (incl. anonymous) sees per-block "N of M filled" (anon-safe `get_slot_availability` RPC); SIGNED-IN users additionally see attendees as "First L." under each block (`get_signup_names` SECURITY DEFINER RPC, migration 013 — auth-required, granted to authenticated only) | **OPEN** | Owner-ordered change (2026-07-05) to the LOCKED public audition page — pending user verify. **PRIVACY LINE (owner-set): never names to anon, never full last names, deliberately NO profile links.** Degrades gracefully pre-013 (counts only); mock derives names from mock signups. |
...
- **Public slot names (migration 013 — applied on staging 2026-07-05; `PASTE_ME_NEXT.sql` now carries migration 014 `org_invites.invited_name` only):** `getSignupNames(showId)` in client.ts — signed-in-only attendee names ("First L.") per audition block via the `get_signup_names` SECURITY DEFINER RPC (requires `auth.uid()`, granted to authenticated only — anon gets counts via `get_slot_availability` and nothing else). Cloud read try/catch → `[]` pre-paste; mock derives from mock signups' `actorName`. QA staging population: `supabase/QA_SEED.sql` (60 dummy actors + 3 org-less makers in auth.users, all `qa.*@overturestage.com`, shared password `OvertureQA2026!`, idempotent) / `supabase/QA_TEARDOWN.sql` (cascade delete + verify count). Staging only, never prod.
