...
| Volunteer dual-path (tables + `claim_volunteer_slot`/`cancel_volunteer_signup` RPCs in migration 010) | **OPEN** | Schema staged 2026-07-05; UI + public `/volunteer/[showId]` guest page are Show Hub part 2. |
...
| Reminder cron (`send-reminders`: audition T-24h/T-2h + 48h offer nudges + rehearsal T-24h/morning-of to CALLED members only, idempotent via `reminder_log`) | **OPEN** | Built; deploy + 15-min cron pending. See `supabase/SETUP_REMINDERS.md`, migrations 008 + 010. Rehearsal scan degrades quietly until 010 is pasted. |
